...
\begin{proof}
  ...
  Sia allora \(\Gamma_0 = leaf(\pi)\cap \Gamma\). Ovviamente \(\Gamma_0\subseteq\Gamma\), \(\Gamma_0\subseteq\Gamma\) e \(\abs{\Gamma_0}<\infty\).
...
\begin{proof}
  ...
  Essendo \(\Gamma_0\) finito sappiamo che \(\Gamma\models\phi\iff\models(\bigwedge\Gamma_0)\to\phi\), e quindi \(\models((\bigwedge_{\phi\in\Gamma_0}\phi)\to\phi)\sigma\) per il teorema di sostituzione.
...
\todo{Perché c'è bisogno di induzione strutturale? La conseguenza logica da parte di un insieme di formule non è definita esattamente come abbiamo definito la preservazione della verità?}
...
\begin{proof}
  ...
  \begin{itemize}
    \item Se vale \(S_P\subseteq S\) o \(S_{\neg P}\subseteq S\), ovvero una tra \(P\) e \(\neg P\) non è presente in nessuna clausola di \(S\), allora \(S\vdash_R \square\) banalmente per monotònicità.\todo{Abbiamo dimostrato la monotònicità?}
