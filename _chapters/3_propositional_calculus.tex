...
Nel capitolo precedente abbiamo definito una logica come una tripla \((\mathcal{L}, \models, \vdash)\), dove \(\mathcal{L}\) è un linguaggio, \(\models\) è la relazione di soddisfacibilità che ne esprime la semantica, e \(\vdash\) è un calcolo, che ci permette di dedurre formule a partire da altre formule, o verificare se una formula è valida o meno.

Per quanto concerne la logica proposizionale, abbiamo definito il linguaggio \(\PL\) e fornito due semantiche. 
Abbiamo visto una forma di calcolo che fa uso delle tabelle di verità, ma ciò non è sufficiente.
...
Vedremo in questo capitolo due tipi di calcolo, un primo detto \keyword{calcolo di deduzione naturale}, più affine al ragionamento umano, ed un secondo, detto \keyword{calcolo di risoluzione}, più affine ad un'implementazione automatica, e che è alla base di molti sistemi di dimostrazione automatica.

Entrambi questi calcoli sono puramente sintattici, e non fanno riferimento alla semantica. 
Tuttavia, le regole di trasformazione che li caratterizzano sono formulate in modo da preservare le proprietà semantiche, e quindi ci permettono di dedurre conseguenze logiche, ovvero di ottenere la stessa relazione di deducibilità che abbiamo definito tramite la semantica, ma in maniera puramente sintattica.

\begin{definition}{Calcolo inferenziale}
  Definiamo un \keyword{calcolo inferenziale} come una coppia \((\Axi, \Rules)\) dove \(\Axi \subseteq \PL\) è un insieme di formule dette \keyword{assiomi}, e \(\Rules\) è un insieme di regole di trasformazione dette \keyword{regole di inferenza}.
...
Ovviamente, come anticipato, non tutte le regole di trasformazione sono regole valide d'inferenza, poiché vorremmo che queste regole preservassero delle proprietà semantiche.

A prescindere da ciò, però, a partire da un calcolo inferenziale \((\Axi, \Rules)\), è possibile definire una relazione di deducibilità.
...
Per quanto riguarda gli assiomi, è possibile usare un insieme minimale di tautologie che potranno essere istanziate in diversi modi tramite il teorema di sostituzione.\footnote{In un certo senso la regola di sostituzione fa parte di \(\Rules\).}
Ad esempio, definendo \(P \lor \neg P\) come assioma, è immediato ottenere \(\lnot P \lor \lnot \lnot P\) come istanziazione in cui \(P\) è mappato in \(\lnot P\).

Il primo calcolo che vedremo, però, non ha assiomi, ma solo regole di inferenza, ed il cui nome è \keyword{calcolo di deduzione naturale}.

\section{Calcolo di deduzione naturale}

Avendo già anticipato l'assenza di assiomi, il calcolo di deduzione naturale è definito esclusivamente dalle regole di inferenza, che sono formulate in modo da catturare il significato semantico degli operatori logici, e quindi ci permettono di dedurre conseguenze logiche.\footnote{
...
\begin{note}{}
  ...
  Inoltre, quando ci sono delle assunzioni che è possibile scaricare\footnote{Un'assunzione è una premessa temporanea che introduciamo per dimostrare qualcosa. \q{Scaricare un'assunzione} significa che, una volta raggiunta la conclusione desiderata sotto quella premessa, possiamo chiudere quella premessa e considerare la conclusione valida senza dover più dipendere da essa.}, queste sono indicate tra parentesi quadre, mentre i puntini di sospensione indicano l'applicazione di un numero finito di regole da quelle assunzioni fino alla conclusione.
\end{note}
...
La regola di introduzione \(\land I\) ci dice che, a partire da due premesse \(A,B\), è possibile concludere \(A\land B\).
Le regole di eliminazione ci dicono che, a partire da una premessa \(A\land B\), è possibile concludere \(A\) (\(\land E^l\)), così come \(B\) (\(\land E^r\)). 

\paragraph{Regole per \(\to\):}

Per quanto riguarda \(\to\), la regola di eliminazione \(\to E\) è abbastanza intuitiva ed è anche conosciuta col nome di \textit{Modus Ponens}, mentre la regola di introduzione \(\to I\) è un po' più complessa.
...
Quello che ci dice è che, se assumendo \(A\) riusciamo a dedurre \(B\), applicando regole di inferenza un numero finito di volte, allora possiamo dedurre \(A \to B\) senza più assumere \(A\), \keyword{scaricando l'assunzione}. Tale regola cattura il procedimento usuale di dimostrazione di un teorema che prevede un'implicazione, ovvero volendo dimostrare \(A \to B\), allora si assume \(A\) e si tenta di dedurre \(B\). In caso affermativo, si potrà concludere \(A \to B\).
...
Quest'ultima regola ci dice che, avendo \(A \lor B\), e se, assumendo \(A\) e \(B\) \textbf{separatamente}, riusciamo a dedurre \(C\) in entrambi i casi, allora possiamo dedurre \(C\) senza più assumere \(A\) e \(B\), ovvero scaricando entrambe le assunzioni nelle rispettive deduzioni.
Questa regola cattura il concetto di dimostrazione per casi: se si vuole dimostrare \(C\) a partire da \(A \lor B\), sarà sufficiente dedurre \(C\) sia nel caso in cui sia vero \(A\) che nel caso in cui sia vero \(B\). In caso affermativo, si potrà concludere \(C\).
...
\begin{definition}{Albero di deduzione ben formato}
  Un \keyword{albero di deduzione} è un albero finito i cui nodi sono etichettati con formule di \(\PL\).
...
In particolare, non procederemo \q{in avanti}, ovvero partendo da un insieme di premesse per poi arrivare a dimostrare la conclusione, bensì \q{a ritroso}: partiremo dalla conclusione e costruiremo l'albero di deduzione partendo dalla radice, ovvero la conclusione stessa, fino a raggiungere le foglie, ovvero le premesse, scaricando\footnote{In realtà, come precedentemente affermato, una premessa viene scaricata nel momento in cui può essere accantonata dopo aver dimostrato la conclusione. In questa costruzione a ritroso, invece, assumeremo di aver scaricato una premessa, che sarà poi esplicitata in una foglia dell'albero.} quest'ultime nel caso sia necessario.
...
\begin{note}{}
  ...
  Formalmente questo processo non è corretto, poiché assume che il nostro calcolo inferenziale catturi la conseguenza logica, ovvero che abbia le proprietà di correttezza e completezza, le quali, senza una dimostrazione formale, non possono essere date per scontate.
...
Dimostrare \(Q\) assumendo \(Q\) è immediato scaricando l'assunzione di \(Q\).
\begin{prooftree}
  \AxiomC{\(\neg P \lor Q\)}
  \AxiomC{\({[Q]}_2\)}
  \UnaryInfC{\(Q\)}
...
Ulteriori esempi ed esercizi di deduzioni naturali sono state riportate nell' appendice \nameref{app_sec:natural_deduction_exercises} per evitare di appesantire troppo questa sezione, ma è consigliabile provarle per prendere confidenza con questo sistema di deduzione.
...
In questo senso, andremo a dimostrare varie proprietà del calcolo di deduzione naturale, che lo mettono in relazione con la conseguenza logica, per garantire che questa tecnica di manipolazione sintattica riesca a mantenere valore semantico, cioè che se una formula è dimostrabile, allora è anche valida, e viceversa.
...
È importante notare che, anche nel caso in cui l'aggiunta di \(\psi\) renda \(\Gamma \cup \set{\psi}\) inconsistente, la proprietà risulta rafforzata: in questo caso sarà possibile dedurre qualsiasi formula a partire da \(\Gamma \cup \set{\psi}\) mediante \(\bot E\). Questa è una caratteristica tipica della logica classica, e non è condivisa da tutte le logiche, come ad esempio le logiche paraconsistenti, in cui l'inconsistenza non porta alla trivialità\footnote{Inteso come banalità. Un classico esempio di situazione triviale è \(A \to B\) con \(A\) falsa.}, ma viene in qualche modo contenuta permettendo di parlare del resto dell'insieme di formule anche in presenza di contraddizioni.

\begin{note}{}
  Questa proprietà si generalizza in \(\Gamma \vdash \phi \implies \Gamma \cup \Gamma' \vdash \phi\), applicandola più volte. La dimostrazione di questa versione più generale, però, segue direttamente da quella della versione più semplice, e quindi non si ritiene necessario riportarla.
\end{note}

\begin{proof}
  Per definizione di deduzione, \(\Gamma \vdash \phi\) se e solo se esiste un albero di derivazione ben formato avente radice etichettata con \(\phi\) e le cui foglie o sono etichettate con formule di \(\Gamma\) o sono assunzioni scaricabili. 
...
\begin{theorem}{Regola del taglio}
  Dati due insiemi di formule \(\Gamma\) e \(\Gamma'\) e due formule \(\phi\) e \(\psi\), tali che \(\Gamma, \psi \vdash \phi\) e \(\Gamma' \vdash \psi\) allora
...
\begin{proof}
  ...
  Infatti per monotònicità valgono sia \(\Gamma \cup \Gamma', \psi \vdash \phi\), che \(\Gamma \cup \Gamma'\vdash \psi\).
...
Dunque, dimostrare la tesi equivale a dimostrare che, per ogni albero di deduzione con conclusione \(\phi\) e premesse \(\Gamma\), si ha \(\Gamma\models\phi\).
Possiamo facilmente vederlo per induzione sulla struttura (o altezza) dell'albero di deduzione.

\begin{proof}
  Procediamo per induzione sull'altezza dell'albero di deduzione.
  
  Il caso base è una deduzione di altezza \(0\), ovvero \(\phi \in \Gamma\). In questo caso, \(\Gamma\models\phi\) è ovviamente verificato.

  A questo punto sia l'altezza maggiore di \(0\). In questo caso avremo che l'albero avrà in radice \(\phi\), dedotta da una certa regola \(\Rule \in \Rules\), con figli \(\psi_1,\ldots,\psi_k \in \PL\), ognuno di questi a sua volta radice di un sottoalbero di altezza strettamente minore di quella dell'albero di deduzione di \(\phi\). Per ipotesi induttiva, \(\Gamma\models\psi_i\) per ogni \(i=1,\ldots,k\).

  A questo punto, quindi, ci basta mostrare per casi che, per ogni regola di deduzione naturale, se \(\set{\psi_1,\ldots,\psi_k} \vdash_{\Rule} \phi\) allora \(\set{\psi_1,\ldots,\psi_k} \models_{\Rule} \phi\).

  \begin{description}
    \item[\(\to E\).] Abbiamo \(\Gamma\models\psi\), per ipotesi induttiva, e \(\Gamma\models\psi\to\phi\), per ipotesi induttiva in modo specifico per questa regola.
    In altre parole, ogni modello che soddisfa \(\Gamma\) soddisfa anche \(\psi\) e \(\psi\to\phi\).
      
    Dalla semantica dell'implicazione, abbiamo che, se un modello \(m\) soddisfa \(\psi\to\phi\) allora o non soddisfa \(\psi\) o, se la soddisfa, allora soddisfa \(\phi\). Ma poiché \(m\) soddisfa \(\psi\), allora \(m\) deve soddisfare anche \(\phi\). Dunque ogni modello che soddisfa \(\Gamma\) soddisfa anche \(\phi\), e quindi \(\Gamma\models\phi\).
    ...
    \item[] Per questioni di brevità non verranno trattate le regole rimanenti. Gli altri casi sono analoghi a quelli già trattati, e possono essere dimostrati con un ragionamento simile a quello fatto per le regole già trattate.
  \end{description}
\end{proof}

Conclusa la dimostrazione di correttezza, possiamo passare al verso più interessante, ovvero quello di completezza, che ci dice che tutto ciò che è conseguenza logica è anche deducibile, e che quindi il calcolo di deduzione naturale è in grado di catturare tutta la conseguenza logica.
...
Per fare ciò, però, dobbiamo introdurre un concetto simile a quello di denotazione --- ed in un certo senso duale --- ovvero la \keyword{teoria} di un modello.

\begin{definition}{Teoria di un Modello}
...
\begin{proof}
  ...
  A questo punto, dobbiamo mostrare che \(\Sigma^* = \Th{m^*}\), ovvero che per ogni formula \(\phi\), si abbia \(\phi \in \Sigma^* \iff m^* \models \phi\).
  Possiamo procedere per induzione sulla struttura di \(\phi\) per mostrare che \(\phi \in \Sigma^*\) se e solo se \(m^*\models\phi\).

  Tuttavia, per fare ciò, dobbiamo prima mostrare che \(\Sigma^*\) è chiuso per deduzione, ovvero che se \(\Sigma^*\vdash\phi\) allora \(\phi\in\Sigma^*\).
  ...
  Adesso, possiamo procedere con la dimostrazione per induzione strutturale.
  Il caso base è una formula atomica \(p\). Se \(p\in\Sigma^*\), allora \(p\in m^*\) per costruzione, e quindi \(m^*\models p\) e viceversa.
  A questo punto, sia \(\phi\) una formula non atomica. In questo caso, \(\phi\) è costruita a partire da formule di lunghezza minore tramite l'applicazione di una regola di formazione. L'ipotesi induttiva è dunque che per ogni \(\psi\) di lunghezza minore di \(\phi\), si abbia \(\psi \in \Sigma^*\) se e solo se \(m^*\models\psi\).
  \begin{description}
    \item[\(\phi = \neg\psi\):] Dalla semantica della negazione, \(m^*\models \phi \iff m^* \not\models \psi\), e per ipotesi induttiva \(m^*\not\models \psi\iff \psi \notin\Sigma^*\). Dalla definizione di massimalità di \(\Sigma^*\) si ha che \(\psi\notin\Sigma^*\) se e solo se \(\neg\psi\in\Sigma^*\), ovvero \(\phi\in\Sigma^*\).
    \item[\(\phi = \psi_1 \land \psi_2\):]
        ...
        Ma essendo che 
        \[\psi_1,\psi_2 \vdash_{\land I} \psi_1\land\psi_2\]
        e che 
        \[\psi_1\land\psi_2\vdash_{\land E} \psi_1 \text{ e } \psi_1\land\psi_2\vdash_{\land E} \psi_2\]
        dalla chiusura di \(\Sigma^*\) per deduzione si ha la tesi.
    \item[\(\phi = \psi_1 \lor \psi_2\):]
        Dalla semantica della disgiunzione, \(m^*\models \phi\) se e solo se \(m^*\models \psi_1\) o \(m^*\models \psi_2\). Per ipotesi induttiva, \(m^*\models \psi_1\) se e solo se \(\psi_1\in\Sigma^*\), e \(m^*\models \psi_2\) se e solo se \(\psi_2\in\Sigma^*\). Ci basta quindi mostrare che \(\psi_1\in\Sigma^*\) o \(\psi_2\in\Sigma^*\iff \psi_1\lor\psi_2\in\Sigma^*\).
        Per la direzione \(\implies\) si ha facilmente che 
        \[\psi_1 \vdash_{\lor I} \psi_1\lor\psi_2 \text{ e } \psi_2 \vdash_{\lor I} \psi_1\lor\psi_2\]
        e dunque dalla chiusura di \(\Sigma^*\) per deduzione si \(\psi_1\lor\psi_2\in\Sigma^*\).
        Per la direzione \(\impliedby\) è sufficiente che una tra \(\psi_1\) e \(\psi_2\) sia in \(\Sigma^*\). Se per assurdo nessuna delle due fosse in \(\Sigma^*\) però, per massimalità di \(\Sigma^*\) si avrebbe che sia \(\neg\psi_1\) che \(\neg\psi_2\) sono in \(\Sigma^*\). Ma allora 
        \[\psi_1\lor\psi_2,\neg\psi_1,\neg\psi_2 \vdash_{\lor E} \bot \implies \Sigma^*\vdash \bot\]
        assurdo per consistenza di \(\Sigma^*\).
      \item[\(\phi = \psi_1 \to \psi_2\):]
        Questo caso può essere facilmente ricostruito a partire dai casi già trattati, utilizzando la semantica di \(\to\) e le regole di deduzione ad esso associate.
  \end{description}
\end{proof}

\begin{theorem}[namedlabel={thm:coherence_and_model-existence}{Coerenza e Esistenza del modello}]{Coerenza e Esistenza del modello}
  Dato \(\Sigma\subseteq\PL\), si ha che:
  
  \(\Sigma\) è consistente \(\iff\Sigma\) è soddisfacibile.
\end{theorem}

Un insieme di formule \(\Sigma\) è \keyword{soddisfacibile} quando esiste un modello \(m^*\) tale che \(m^*\models\Sigma\).

\begin{proof}
  Per definizione di consistenza si ha che \(\Sigma \not\vdash \bot\), ovvero non esiste alcun albero di deduzione con radice \(\bot\) con foglie che sono o assunzioni scaricate o formule appartenenti a \(\Sigma\).

  Per mostrare che se \(\Sigma\) consistente allora \(\Sigma\) è soddisfacibile, dobbiamo andare a costruire un modello \(m^*\) che soddisfa \(\Sigma\) sapendo che le formule di \(\Sigma\) sono consistenti.
...
\begin{proof}
  ...
  Di conseguenza, la proposizione \(\Gamma\models\phi \implies \Gamma\vdash\phi\) equivale alla proposizione \(\Gamma \cup \set{\neg\phi}\) è insoddisfacibile \(\implies\) \(\Gamma \cup \set{\neg\phi}\) è inconsistente.

  A questo punto, dimostriamo quest'ultima proposizione per contrapposizione, ovvero che, preso un qualsiasi insieme \(\Gamma\), si osserva \(\Gamma\) consistente \(\implies\) \(\Gamma\)soddisfacibile.
  Per fare ciò, è sufficiente il Teorema di \Namedonlyhref{thm:coherence_and_model-existence}, che ci dice che un insieme di formule consistente è soddisfacibile. Di conseguenza, se \(\Gamma\cup\set{\neg\phi}\) è insoddisfacibile, allora è inconsistente, ovvero se \(\Gamma\models\phi\) allora \(\Gamma\vdash\phi\).
\end{proof}

Esistono infine un ultimo paio di importanti risultati riguardo al calcolo di deduzione naturale della logica proposizionale. 
Entrambi semplificano, in un certo senso, la trattazione di questo calcolo: il primo sul fronte semantico, riducendo la grandezza dello spazio dei modelli da considerare; il secondo sul fronte sintattico, permettendo di \qi{modularizzare} le dimostrazioni, rendendole più generali ed astratte, nonché di più ampio utilizzo.
...
\[\Unsat[\Gamma] \iff \exists \Gamma_0 \subseteq \Gamma, \abs{\Gamma_0} < \infty\st \Unsat[\Gamma_0]\]
e per dualità
\[\Sat[\Gamma] \iff \forall \Gamma_0 \subseteq \Gamma, \abs{\Gamma_0} < \infty \st \Sat[\Gamma_0]\]
...
\begin{proof}
  Per dimostrare la prima direzione, ovvero che \(\Gamma\models\phi\) implica l'esistenza di un sottoinsieme finito \(\Gamma_0\subseteq\Gamma\) tale che \(\Gamma_0\models\phi\), iniziamo notando che dal teorema di completezza si ha
  \[\Gamma \models \phi \implies \Gamma \vdash \phi\]
  Ovvero esiste una deduzione \(\pi\) di \(\phi\) da \(\Gamma\).
  ...
  Nell'altro verso è ovvio per monotònicità che
  \[\Gamma_0\models\phi\overset{\text{comp}}{\implies}\Gamma_0\vdash\phi\overset{\text{mon}}{\implies}\Gamma\vdash\phi\overset{\text{corr}}{\implies}\Gamma\models\phi\]
\end{proof}

Arriviamo infine ad un teorema che generalizza il \Namedcref{thm:substitution}, che riguarda la sostituzione nella conseguenza logica invece che nella validità.
...
\begin{theorem}{Invarianza della Conseguenza Logica rispetto alla Sostituzione}
  ...
  con \(\Gamma\sigma = \set{\psi\sigma}[\psi\in\Gamma]\).
\end{theorem}
...
\begin{proof}
  ...
  Dunque \(\Gamma_0\sigma\models\phi\sigma\) per il teorema di deduzione semantica e quindi \(\Gamma\sigma\models\phi\sigma\) per monotònicità.
\end{proof}
...
Il calcolo di deduzione naturale è un calcolo molto potente, che, come il nome suggerisce, risulta naturale per la dimostrazione di teoremi, poiché le regole di deduzione si allineano molto bene con le tecniche di dimostrazione che siamo abituati ad utilizzare.

Tuttavia, il calcolo naturale non si presta bene all'automatizzazione, poiché la ricerca di una deduzione per una formula con un numero elevato di regole da poter applicare risulta difficile da gestire in modo algoritmico.

Introduciamo, quindi, un calcolo di deduzione alternativo, più adatto all'automatizzazione, poiché composto da un'unica regola di deduzione, il \keyword{calcolo di risoluzione}. La presenza di un unica regola di deduzione elimina la necessità di dover scegliere quale regola applicare, \q{sostituendola} con la scelta delle formule da utilizzare per applicare la regola, comportando una semplificazione significativa per la ricerca di una deduzione.

La prima versione del calcolo di risoluzione fu proposta da J. A. Robinson nel 1965 e da allora è stata oggetto di numerose estensioni e miglioramenti.

Tale calcolo non si concentra sulla conseguenza logica, ma sull'inconsistenza. 
Nonostante ciò, e possibile decidere \(\Gamma\models\phi\) decidendo se \(\Gamma\cup\set{\neg\phi}\) è insoddisfacibile.
...
\begin{definition}{Forma clausale}
  ...
  In altre parole, una formula è in forma clausale se è un insieme di insiemi di letterali.\footnote{Abbiamo dato la definizione di letterale quando abbiamo introdotto le forme normali, nel secondo capitolo.}
\end{definition}

Ovviamente, essendo insiemi, l'ordine degli elementi non conta. 
Ogni insieme interno non può contenere due volte lo stesso letterale, e l'insieme esterno non può contenere due volte lo stesso insieme di letterali.

Esiste una corrispondenza biunivoca tra formule in forma clausale e formule proposizionali in forma normale congiuntiva (\(\CNF\)). 
...
Inoltre, dato un insieme di formule \(\Gamma\subseteq\CNF\), è possibile unirle in un'unica formula \(\bigwedge_{\phi\in\Gamma}\phi \in \CNF\) applicando una congiunzione. Analogamente, è possibile unire più forme clausali in un'unica forma clausale applicando l'unione. 

Notiamo che una forma clausale \(F\) è soddisfatta da un modello \(m\), ovvero \(m\models F\), se e soltanto se, per ogni insieme di letterali \(C \in F\), \(m\models C\) ovvero \(m\models l\) per qualche \(l \in C\).
In breve, \(m\models F \iff \forall C \in F ,\exists l \in C\st m\models l\). 

\(F = \emptyset\) è detta \keyword{forma clausale banale} ed è valida, poiché non c'è nessuna clausola che un modello possa non soddisfare.
È importante notare che la forma clausale vuota \(F=\emptyset\) non coincide con la \keyword{clausola vuota} \(C=\emptyset\), tipicamente indicata con \(\square\). 
Infatti, per la definizione di soddisfacibilità, \(m\not\models \square\) per ogni \(m\), e quindi \(\square\) è insoddisfacibile, con conseguente insoddisfacibilità di qualsiasi \(F\) che contenga \(\square\), al contrario di \(F=\emptyset\), che è soddisfatta da ogni \(m\).
...
\begin{example}{}
  ...
  D'ora in poi, assumeremo che le forme clausali non contengano clausole banali.
\end{example}

Per definire formalmente il calcolo di risoluzione, ovvero definire la sua unica regola di deduzione, anch'essa chiamata risoluzione, è necessario prima introdurre una proprietà delle clausole, che sarà la condizione di applicabilità della regola di risoluzione. 
Stiamo parlando del \keyword{contrasto} (o \keyword{clash}).
...
In altre parole, due clausole sono in clash se contengono un letterale su cui non sono concordi, ovvero se una clausola ha un letterale e l'altra ha il suo complementare. 
Intuitivamente, per soddisfare entrambe le clausole, è necessario che almeno una delle due \qi{rinunci} ad avere ragione su quel letterale: non è permesso che il modello soddisfi un letterale e il suo complementare ed è quindi necessario scegliere una sola versione di questo.
Non sappiamo a priori quale delle due clausole rinuncerà al letterale in clash, ma sappiamo che almeno un altro letterale (diverso da quello in clash) in una delle due clausole deve essere soddisfatto.
Tale ragionamento è proprio ciò che caratterizza la regola di risoluzione.
...
\begin{definition}{Regola di risoluzione}
  ...
  \[\infer[\text{Risoluzione}]{R = (C_1 \cup C_2)\setminus\set{l, \tilde{l}}}{C_1 \quad C_2}\]

\end{definition}

La regola di risoluzione risolve il clash eliminando i letterali di clash, \(l\) e \(\tilde{l}\), dalle due clausole \(C_1\) e \(C_2\) e, successivamente, unendo le due clausole in un'unica clausola \(R\), detta risolvente.
Quindi, il risolvente \(R\) sarà una clausola avente tutti i letterali delle due clausole iniziali tranne i letterali in clash. 
Poiché esso è una clausola disgiuntiva di letterali, il risolvente corrisponde semanticamente al richiedere che sia soddisfatto almeno un letterale, escludendo quelli di clash, tra le due clausole in clash, che, intuitivamente, dovrà appartenere alla clausola che non avrà il letterale di clash soddisfatto.

\begin{warning}{}
  La regola di risoluzione elimina solo un clash alla volta: se sono presenti più letterali in clash tra due clausole, un'applicazione della regola di risoluzione risolverà un solo clash. 
  Se così non fosse, come vedremo, la regola di risoluzione non preserverebbe la verità.
\end{warning}
...
Chiaramente, il preservare la verità implica il preservare anche la conseguenza logica.\footnote{Questa cosa risulta banale nella logica proposizionale, ma non sarà scontata nel prim'ordine.}
...
\begin{proof}
  ...
  Consideriamo il caso \(m\models l\). Essendo che \(m\models C_2\), allora \(\exists l' \in C_2\setminus\set{\tilde{l}}\) tale che \(m\models l'\). Dunque \(l' \in R\), ovvero \(m\models R\).
  Nel caso in cui \(m \models \tilde{l}\), essendo che \(m\models C_1\), allora \(\exists l' \in C_1\setminus\set{l}\) tale che \(m\models l'\). Dunque \(l' \in R\), ovvero \(m\models R\).
\end{proof}

Per quanto riguarda il verso opposto, ovvero il passaggio dal risolvente alle clausole in clash, l'unica proprietà che è preservata è la soddisfacibilità, ovvero se \(R\) risolvente di \(C_1\) e \(C_2\) su \(l\) è soddisfacibile, allora anche \(C_1\) e \(C_2\) sono soddisfacibili. Tale proprietà è ovviamente più debole della preservazione della verità, poiché l'unica garanzia che fornisce è che, se esiste un modello in cui \(R\) è soddisfatto, allora ne esiste uno, potenzialmente diverso, in cui \(C_1\) e \(C_2\) sono congiuntamente soddisfatte.

Non è possibile richiedere di più, poiché \(R\) contiene un numero di proposizioni atomiche potenzialmente inferiore (strettamente) sia di \(C_1\) che di \(C_2\)\footnote{Ad esempio, si considerino le clausole \(C_1 = \set{A, \neg A, B, C}\) e \(\set{A, B, C}\) in clash su \(A, \neg A\). Il risolvente è \(R=\set{A,B,C}\) e contiene tutte le proposizioni atomiche presenti nelle clausole di partenza. In questo esempio, il risultante conserva tutte le proposizioni atomiche delle clausole in clash. D'altra parte, un caso più comune potrebbe essere \(C_1'=\set{A, B, C}\) e \(C_2'=\set{\neg A, B, C}\) in clash su \(A\), con risolvente \(R=\set{B, C}\), mancante della proposizione atomica \(A\).}, e di conseguenza gli assegnamenti che soddisfano \(R\) potrebbero essere assegnamenti non validi per \(C_1\cup C_2\). 

Adesso, mostreremo che, dato \(\alpha\in \set{0,1}^{\AP[R]}\) tale che \(\Val{\alpha}[R] = 1\), esiste un assegnamento \(\alpha'\in\set{0,1}^{\AP[C_1]\cup\AP[C_2]}\) tale che \(\Val{\alpha'}[C_1] = 1\) e \(\Val{\alpha'}[C_2] = 1\), ovvero che, se \(R\) è soddisfacibile, allora lo sono anche \(C_1\) e \(C_2\), contemporaneamente.

\begin{proposition}{Soddisfacibilità per la regola di risoluzione}
...
\begin{proof}
  ...
  Senza perdita di generalità, supponiamo \(l \in C_1\) e \(\tilde{l} \in C_2\).
  ...
  \[\alpha'(P) = \begin{cases}
              ...
              \end{cases}
  \]
  soddisfa ancora entrambe, poiché \(C_1\) sarà soddisfatta da \(l'\), mentre \(C_2\) sarà soddisfatta da \(\tilde{l}\), poiché il suo assegnamento è costruito per essere soddisfatto qualunque sia la sua \q{polarità}.\footnote{L'assegnamento è costruito in modo tale da soddisfare la clausola che contiene \(\tilde{l}\) a prescindere dalla valutazione di \(\tilde{l}\). Per \q{polarità} si intende il valore binario della valutazione.}
...
Avendo ora un insieme di regole di inferenza, in questo caso composto da una regola sola, e un insieme di assunzioni, in questo caso la forma clausale, possiamo andare a definire il calcolo di deduzione di risoluzione attraverso la definizione di deduzione.

\begin{definition}{Deduzione di risoluzione}
  Sia \(F\) una forma clausale e \(C\) una clausola. Una deduzione di risoluzione \(F\vdash_{\Rule }C\) è un albero binario finito, la cui radice è etichettata con \(C\), tutte le foglie sono etichettate con clausole di \(F\) e ogni nodo interno è etichettato con il risolvente dei suoi figli.
\end{definition}

\begin{note}{}
  Poiché il calcolo di risoluzione possiede una sola regola di inferenza, \(\Rules = \set{\Rule}\), ovvero la regola di risoluzione, indicheremo le deduzioni di risoluzione semplicemente con \(\vdash_{\Rule}\) e le deduzioni naturali con \(\vdash_{ND}\).
\end{note}

Poiché abbiamo dimostrato che la regola di risoluzione preserva la verità, e quindi la conseguenza logica, è facile dimostrare, per induzione strutturale (induzione sull'altezza dell'albero di deduzione di risoluzione), la correttezza del calcolo, ovvero che data \(S\) forma clausale e \(C\) clausola, allora \(S\vdash_{\Rule} C \implies S\models C\).

Sarà possibile dimostrare anche una forma più debole della completezza, legata alla soddisfacibilità, ovvero che \(S\not\vdash_R \square \implies \Sat[S]\), oppure, equivalentemente, \(\Unsat[S] \implies S\vdash_R \square\). Come già anticipato, non sarà possibile dimostrare la totale completezza, perché esistono conseguenze logiche di clausole che non sono in alcun modo generabili dal calcolo.

\begin{example}{}
  Consideriamo infatti la forma clausale \(F = \set{\set{A},\set{B}}\). Dall'interpretazione semantica data, è ovvio che \(R=\set{A,B}\) sia conseguenza logica di \(F\). Tuttavia, poiché non vi sono letterali in clash in \(F\), non è possibile applicare la regola di risoluzione, impedendo di derivare alcunché.

  È possibile però in parte aggirare questo problema utilizzando la definizione alternativa di conseguenza logica, ovvero \(S\models C \iff S\cup\set{\neg C} \in \Unsat\).

  Applicando tale riformulazione all'esempio di prima, potremo dimostrare \(F\models R\) cercando di mostrare che \(F' = \set{\set{A}, \set{B}, \set{\neg A, \neg B}}\) è insoddisfacibile. In questo caso, giacché \(C_1 = \set{A}\) e \(C_2 = \set{\neg A, \neg B}\) sono in clash su \(A\), è possibile applicare la regola di risoluzione per dedurre \(C_3 = \set{\neg B}\), da cui è possibile dedurre \(C_4 = \square\) con \(C_2\), che è in clash con \(C_3\) su \(B\). 
  Dunque, per la correttezza del calcolo, \(F'\) è insoddisfacibile, e quindi \(F\models R\) per la definizione alternativa di conseguenza logica.
\end{example}

Utilizzando la riformulazione presentata nell'esempio, e questa completezza debole, è possibile risolvere il problema di conseguenza logica usando la risoluzione, passando per il problema di soddisfacibilità.
...
L'algoritmo di risoluzione prende in input una forma clausale \(S\) e restituisce \(\Unsat\), se \(S\) è insoddisfacibile, \(\Sat\) se è soddisfacibile.
L'algoritmo si serve di due insiemi: \(S_0\), ovvero l'insieme, in evoluzione, delle clausole; \(E\), l'insieme delle coppie in clash già considerate. 

L'algoritmo prende una coppia di clausole in clash \((C_1,C_2)\in (S_0 \times S_0)\setminus E\), le risolve, aggiunge il risolvente (se esso non è banalmente vero e.g. \(R=\set{Q, \neg Q}\)) a \(S_0\) e la coppia ad \(E\), fino al momento in cui non si verifica \(R=\square\) (in tal caso, la forma clausale è necessariamente insoddisfacibile e ci si può fermare) oppure sono state considerate tutte le coppie in clash. 

Al termine del ciclo, se \(R=\square\), allora l'algoritmo restituisce \(\Unsat\), altrimenti \(\Sat\).

Per quanto riguarda la correttezza dell'algoritmo, questa risulta quasi scontata dal fatto che è l'esatta trasposizione di una deduzione di risoluzione. 
Riguardo alla terminazione, invece, la cosa è meno scontata, in quanto è e dev'essere valutata sull'insieme \(S_0\), il quale non è statico, ma cresce dinamicamente durante l'esecuzione. In linea di principio quindi, senza mostrare che esista un limite superiore al numero di clausole che è possibile generare mediante risoluzione, non è possibile garantire la terminazione di questo algoritmo. Chiaramente, il numero massimo di iterazioni sarà pari a \(\abs{S_0}^2\), ma è necessario mostrare che \(\abs{S_0}\) è limitato, e quindi che esiste un limite superiore al numero di iterazioni.
Fortunatamente, tale limite superiore esiste, e ne vedremo la dimostrazione formale dopo un esempio di applicazione dell'algoritmo descritto finora.
...
\begin{example}{}
  ...
  In primo luogo, abbiamo diverse scelte. Prendiamo, ad esempio, le clausole \(2\) e \(4\) in clash su \(G\). Il risolvente è \(\set{\neg R, L}\) e lo indichiamo con \(6\).
  Da \(1\) e \(4\) in clash su \(G\), otteniamo \(\set{\neg C, R}\), che indichiamo con \(7\).
  Da \(3\) e \(7\) otteniamo \(\set{L, R}\), che indichiamo con \(8\).
  Da \(6\) e \(8\) otteniamo \(\set{L}\), che indichiamo con \(9\).
  Da \(5\) e \(9\) otteniamo \(\square\).

  Ovviamente l'esecuzione descritta non è l'unica possibile, avendo avuto libera scelta sulle risoluzioni. Ad esempio, avremmo potuto iniziare da \(1\) e \(2\) in clash su \(R\) per ottenere \(\set{\neg C, G, L}\), che adesso indichiamo con \(6\).
  È possibile risolvere \(6\) con \(3\) ottenendo \(\set{G, L}\), che indichiamo con \(7\).
  A questo punto, risolvendo prima con \(4\), e poi il risolvente con \(5\), otteniamo \(\square\).
\end{example}

Come già menzionato precedentemente, una singola applicazione della regola di risoluzione porta all'eliminazione di un solo letterale alla volta.

Non è possibile estendere banalmente la regola per permettere la risoluzione di più letterali contemporaneamente, poiché in tal caso è facile costruire controesempi in cui non è preservata la verità. Prendiamo infatti le clausole \(C_1 = \set{A, B}\) e \(C_2 = \set{\neg A, \neg B}\). Se fosse possibile risolvere entrambe le coppie di letterali in clash contemporaneamente, allora sarebbe possibile dedurre la clausola vuota \(\square\) da \(C_1\) e \(C_2\), che è insoddisfacibile, mentre invece \(C_1\) e \(C_2\) sono congiuntamente soddisfacibili da un assegnamento che assegna \(1\) ad \(A\) e \(0\) a \(B\), o viceversa. Andando ad applicare la regola di risoluzione in modo corretto, è possibile dedurre \(\set{A,\neg A}\) da \(C_1\) e \(C_2\) risolvendo su \(B\), ottenendo una clausola banalmente soddisfacibile \(\set{A, \neg A}\), essendo tautologica.

Fatte queste precisazioni, possiamo ora dimostrare la terminazione dell'algoritmo di risoluzione, mostrando che esiste un limite superiore al numero di clausole che il calcolo di risoluzione può generare, limitando quindi la grandezza dell'insieme \(S_0\) su cui viene valutata la condizione di terminazione.

\begin{theorem}{Terminazione dell'algoritmo di risoluzione}
  \(\textsc{Resolution}(S)\) termina sempre e l'insieme di saturazione \(S_0\) avrà al più \(3^{\abs{\AP[S]}}\) clausole.
\end{theorem}

\begin{proof}
  Sappiamo che l'insieme di clausole in ingresso \(S\) è finito. Sia dunque \(\AP[S] = \set{P \in \AP[\phi]}[\phi \in S]\) l'insieme di proposizioni atomiche che compaiono in \(S\). Chiaramente \(\AP[S]\) è finito, poiché \(S\) è finito e ogni formula contiene un numero finito di proposizioni atomiche. Infatti, essendo \(\AP[\phi] = \AP \cap Sub(\phi)\) e, come mostrato precedentemente \(\abs{Sub(\phi)} \leq \abs{\phi}\) si avrà che
  \[\forall \phi \in S, \abs{\AP[\phi]} \leq \abs{\phi} \implies \abs{\bigcup_{\phi \in S} \AP[\phi]} \leq \sum_{\phi \in S} \abs{\phi}\]

  Inoltre, avendo escluso le clausole banali, si ha che \(\forall C \in S, \abs{C}\leq\abs{\AP[S]}\). Questo poiché ogni clausola è un insieme di letterali, che può essere presente o in forma positiva o in forma negativa, ma non entrambe, e quindi per ogni proposizione atomica in \(\AP[S]\) al più una delle due forme può essere presente in una clausola.
...
\begin{proof}
  ...
  \[\neg P \notin C \implies C\setminus\set{P} \in S_P \implies \exists l \in C\setminus\set{P} : \Val{\alpha^P}[l] = \Val{\alpha}[l] = 1\]
  ...
  \begin{itemize}
    \item Se vale \(S_P\subseteq S\)\footnote{\(S,S_P\) sono forme clausali. Pertanto, \(S_P \subseteq S\) vuol dire che \(S_P\) contiene alcune clausole (o tutte) di \(S\) e non ne contiene nessuna che non sia in \(S\).} o \(S_{\neg P}\subseteq S\), cioè, rispettivamente, \(P\) non appare in nessuna clausola di \(S\) oppure \(\neg P\) non appare in nessuna clausola di \(S\), allora \(S\vdash_R \square\) banalmente per monotònicità.
    ...
    In altre parole c'è almeno una foglia per albero di deduzione che è stata ottenuta dalla sottrazione del letterale.
    ...
    \begin{center}
      ...
      \end{minipage}
    \end{center}
    In questo modo, abbiamo costruito una nuova deduzione di risoluzione che, a partire da \(S\) essendo tutte le foglie etichettate con clausole in \(S\), deduce \(\set{P}\).
...
\begin{example}{}
  ...
  Dobbiamo adesso aggiungere \(P\) (\(\neg P\)) a tutte le clausole di \(S_P\) ottenute mediante sottrazione di \(P\) (\(\neg P\)) da clausole di \(S\), e propagare le aggiunte nei risolventi.
  ...
  A questo punto possiamo dedurre \(\square\) facilmente col clash di \(P\) e \(\neg P\), ottenendo un albero di deduzione con radice \(\square\) e foglie in \(\AP{S}\), dimostrando, con questo procedimento, che \(S\) è \(\Unsat\).
