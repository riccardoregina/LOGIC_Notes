...
Per quanto riguarda gli assiomi, è possibile usare un insieme minimale di tautologie che potranno essere istanziate in diversi modi tramite il teorema di sostituzione.\footnote{In un certo senso la regola di sostituzione fa parte di (R).}
...
\begin{note}{}
  Questa proprietà si generalizza in \(\Gamma \vdash \phi \implies \Gamma \cup \Gamma' \vdash \phi\), applicandola più volte. La dimostrazione di questa versione più generale però segue direttamente da quella della versione più semplice, e quindi non è necessario farla esplicitamente.
\end{note}
...
\begin{proof}
  ...
  \begin{description}
    ...
    \item[\(\phi = \psi_1 \lor \psi_2\):]
        ...
        \[\psi_1\lor\psi_2,\neg\psi_1,\neg\psi_2 \vdash_{\lor E} \bot \implies \Sigma^*\implies \bot\]
        assurdo per consistenza di \(\Sigma^*\).
      \item[\(\phi = \psi_1 \to \psi_2\):]
        Questo caso può essere facilmente ricostruito a partire dai casi già trattati, utilizzando la semantica di \(\to\) e le regole di deduzione a esso associate.
  \end{description}
\end{proof}
...
\begin{proof}
  ...
  Dunque per mostrare che \(\Sigma\) consistente implica che \(\Sigma\) è soddisfacibile, dobbiamo andare a costruire un modello \(m^*\) che soddisfa \(\Sigma\) sapendo che le formule di \(\Sigma\) sono consistenti.
...
\begin{proof}
  ...
  Ma per fare ciò è sufficiente il Teorema di \Namedonlyhref{thm:coherence_and_model-existence}, che ci dice che un insieme di formule consistente è soddisfacibile. Di conseguenza, se \(\Gamma\cup\set{\neg\phi}\) è insoddisfacibile, allora è inconsistente, ovvero se \(\Gamma\models\phi\) allora \(\Gamma\vdash\phi\).
\end{proof}
...
Non è possibile richiedere di più poiché \(R\) contiene un numero di proposizioni atomiche potenzialmente strettamente inferiore sia di \(C_1\) che di \(C_2\), e di conseguenza gli assegnamenti che soddisfano \(R\) non sono assegnamenti validi per soddisfare \(C_1\) e \(C_2\). Andiamo però a mostrare che dato \(\alpha\in \set{0,1}^{\AP[R]}\) tale che \(\Val{\alpha}[R] = 1\), esiste un assegnamento \(\alpha'\in\set{0,1}^{\AP[C_1]\cup\AP[C_2]}\) tale che \(\Val{\alpha'}[C_1] = 1\) e \(\Val{\alpha'}[C_2] = 1\).
...
\begin{note}{}
  Essendo che in risoluzione \(\Rules = \set{\Rule}\), con \(\Rule\) la regola di risoluzione, per distinguere tale calcolo da quello di deduzione naturale indicheremo le deduzioni di risoluzione con \(\vdash_{\Rule}\) invece che con \(\vdash\). Quando sarà necessario esplicitare l'utilizzo del calcolo di deduzione naturale invece, per dissipare ogni ambiguità, verrà utilizzato \(\vdash_{ND}\).
\end{note}

Essendo che la regola di risoluzione preserva la verità, e quindi la conseguenza logica, è facile vedere per induzione strutturale la correttezza del calcolo, ovvero che dato \(S\) forma clausale e \(C\) clausola, \(S\vdash_{\Rule} C \implies S\models C\).
