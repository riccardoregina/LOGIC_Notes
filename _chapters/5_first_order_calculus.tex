...
\subsubsection*{Dimostrazione di \(\vdash (\exists x\st P(x) \lor \exists x\st R(x))\to (\exists x\st (P(x)\lor R(x)) )\)}
...
\begin{prooftree}\label{prtree:symmetry}
  \AxiomC{\(x=y\)}
  \AxiomC{}
  \RuleLabel{\(Refl\)}
  \UnaryInfC{\(x=x\)}
...
\begin{prooftree}\label{prtree:transitivity}
  \AxiomC{\(x=y\)}
  \RuleLabel{\(Symm\)}
  \UnaryInfC{\(y=x\)}
  \AxiomC{\(y=z\)}
...
\begin{proposition}{Validità di \(Sub_t\)}
  ...
  \[\Gamma \vdash \subs{t}{x_1,\ldots,x_n}{r_1,\ldots,r_n} = \subs{t}{x_1,\ldots,x_n}{s_1,\ldots,s_n}\]
\end{proposition}
...
\begin{proposition}{Validità di \(Sub_\phi\)}
  Sia \(\Gamma = \set{r_1 = s_1, \ldots, r_n = s_n}\), con \(r_1, \ldots, r_n, s_1,\ldots s_n \in \Terms\). Dato \(\phi \in \FOL\) si ha che, se  \(r_1,\ldots,r_n,s_1,\ldots,s_n\) sono liberi per \(x_1,\ldots,x_n\) in \(\phi\) allora
  \[\Gamma \vdash \subs{\phi}{x_1,\ldots,x_n}{r_1,\ldots,r_n} \to \subs{\phi}{x_1,\ldots,x_n}{s_1,\ldots,s_n}\]
...
\begin{proof}
  ...
  Per ipotesi induttiva abbiamo che \(\Gamma, \subs{\psi}{x}{y} \models \phi\) e che \(\Gamma \models \exists x\st \psi\). Chiaramente, per definizione di conseguenza logica, si ha che per entrambe le ipotesi induttive si ha che, per ogni \(M,\sigma\), se \(M,\sigma\) soddisfano le premesse allora soddisfano le rispettive conclusioni.

  In particolare, per ogni \(M,\sigma\), si ha
  \[M,\sigma \models \Gamma \implies M,\sigma\models \exists x\st \psi\]
  e
...
\begin{definition}{Insieme di Henkin}
  \(\Theta \subseteq \FOL\) su linguaggio \(L\) è un insieme di Henkin se, per ogni formula \(\theta\) costruibile in \(L\) della forma \(\theta = \exists x\st \phi(x)\) esiste un termine ground \(t \in L\) tale che \(\exists x\st \phi(x) \to \subs{\phi}{x}{t} \in \Theta\).
\end{definition}
...
\begin{definition}{Espansione di Henkin}
  ...
  \[\Gamma_{n+1} = \Gamma_n \cup \set{\exists x\st \phi_j(x) \to \subs{\phi_j}{x}{c^{n+1}_{j}}}[j \in \N \text{ e } \exists x\st \phi_j(x) \text{ formula chiusa di } L_n]\]
...
\vspace{1cm}
A questo punto, costruiamo, finalmente, il modello \(M^* = \langle D, \Interp\rangle\) la cui teoria \(\Th{M^*} = \set{\phi \in \FOL}[M^* \models \phi] = \Gamma^*\).

Il dominio \(D\), come anticipato, sarà
\[D = \set{t\in \Terms(L')}[\Var(t) = \emptyset]\]
ovvero l'insieme dei termini ground di \(L'\).

Per quanto riguarda la funzione di interpretazione \(\Interp\), questa sarà definita come segue:

\begin{itemize}
  \item Per ogni costante \(c\in \ConstSyms(L')\), \(\Interp(c) = c^{M^*} = c\)
  \item Per ogni simbolo di funzione \(f\in \FuncSyms(L') = \FuncSyms(L)\)\footnote{L'estensione non aggiunge funzioni.} con \(\arty(f) = n\), \(\Interp(f) = f^{M^*} \in D^{D^n}\) tale che \(f^{M^*}(t_1^{M^*}, \ldots, t_n^{M^*}) = f(t_1, \ldots, t_n)\) per ogni \(t_1^{M^*}, \ldots, t_n^{M^*} \in D\). Dunque per qualsiasi termine ground \(t\) si ha che \(\Interp(t) = t\).
  \item Per ogni simbolo di predicato \(P\in \PredSyms(L') = \PredSyms(L)\)\footnote{L'estensione non aggiunge predicati.} con \(\arty(P) = n\), \(\Interp(P) = P^{M^*} =\set{(t_1, \ldots, t_n) \in D^n}[P(t_1, \ldots, t_n) \in \Gamma^*]\).
\end{itemize}
