...
Definite propriamente le regole di deduzione per la logica del prim'ordine, è possibile andare a dimostrare alcune proprietà interessanti riguardo al calcolo di deduzione naturale, in particolare riguardo alla correttezza e alla completezza del calcolo, molte delle quali saranno un analogo o una generalizzazione di quelle viste per la deduzione naturale nella logica proposizionale.
...
Tale dimostrazione giustificherà anche il perché i vincoli di applicabilità per le regole dei quantificatori oltre a essere necessari sono sufficienti. Questo perché vedremo che sarà per l'appunto sufficiente assumere tali vincoli nell'applicazione delle regole per mostrare che le regole preservano la conseguenza logica.
...
Da un punto di vista intuitivo, questo lemma ci sta dicendo cambiare il nome di una variabile in tutte le occorrenze con un altra variabile che non compare nella formula non cambia il significato della formula stessa, e quindi la soddisfacibilità della formula è invariata. Passiamo alla dimostrazione per andare più nel dettaglio.
...
\begin{proof}
  ...
  Essendo che \(y\notin \Var(\phi)\implies y\) libero per \(x\) in \(\phi\).
  ...
  Essendo che tutte le relazioni sono equivalenze, è mostrata la tesi.
\end{proof}
...
Per la logica proposizionale abbiamo dimostrato questo teorema passando alla condizione equivalente \(\Gamma \cup \set{\neg \phi} \subseteq \Unsat \implies \Gamma \cup \set{\neg \phi} \vdash \bot\), che per contrapposizione equivale a \(\Gamma \cup \set{\neg \phi}\not\vdash \bot \implies \Gamma \cup \set{\neg \phi} \subseteq \Sat\). Quindi se dimostrando che dato \(\Gamma\) consistente si ha \(\Gamma \subseteq \Sat\) per un qualsiasi \(\Gamma\) abbiamo la tesi. Sempre nella logica proposizionale il procedimento seguito è stato quello di  costruire un sovrainsieme di \(\Gamma\) massimalmente consistente, che corrispondeva alla teoria di un modello, il quale soddisfacendo il sovrainsieme massimale, soddisfaceva anche \(\Gamma\). La costruzione avveniva mediante enumerazione delle formule, aggiungendo ogni formula all'insieme se l'insieme risultante rimaneva consistente, e analizzando il limite di questa successione notando che era massimalmente consistente.

Per la logica del prim'ordine però il concetto di modello è più complesso, poiché a differenza della logica del prim'ordine in cui un modello non è altro che un insieme di proposizioni atomiche assunti come vere, ma bensì una struttura con un dominio e una funzione di interpretazione. Di conseguenza, la costruzione del modello dall'insieme massimale di formule non è più così immediata, essendo due oggetti fondamentalmente diversi. Per ovviare a questo problema, essendo che dalla prospettiva della logica gli oggetti in cui i simboli sono interpretati sono irrilevanti, è possibile pensare di costruire un modello partendo da un insieme di formule banalizzando al massimo la funzione di interpretazione, ovvero facendo si che il dominio del modello contenga un oggetto per ogni termine ground e interpretando ognuno di questi termini in tale oggetto.

Questa costruzione è però non banale, e non può essere applicata in automatico a qualsiasi linguaggio poiché potrebbe accadere che la signature che stiamo utilizzando non contenga sufficienti simboli per popolare il dominio del modello, costringendoci a usare lo stesso oggetto per interpretare più termini, con il rischio di non riuscire a soddisfare tutte le formule del sovrainsieme massimale. Consideriamo ad esempio il linguaggio \(L=\langle \set{c}, \emptyset, \set{P}\rangle\) e la teoria \(\Gamma = \set{\neg P(c), \exists x\st P(x)}\). Chiaramente esiste un modello di \(\Gamma\), ad esempio \(M = \langle \set{d,d'}, \Interp\rangle\) con \(\Interp(c) = d\) e \(\Interp(P) = \set{d'}\). Tuttavia, essendo che tutti il linguaggio contiene un solo simbolo per formare termini sarà possibile costruire da esso solo domini con un solo elemento, che non potranno mai essere sufficienti a soddisfare \(\Gamma\).

Intuitivamente, il problema deriva dalle formule esistenziali, o equivalentemente dalla negazione di universali, poiché queste richiedono un testimone nel dominio per essere soddisfatte senza usare termini per riferirsi a tali testimoni.
Dobbiamo quindi avere modo di arricchire il linguaggio in modo da poter rappresentare i testimoni necessari a soddisfare le formule esistenziali. Quello che faremo è quindi aggiungere costanti al linguaggio che fungeranno da testimoni, aggiungendo alla teoria degli assiomi che leghino le formule esistenziali precedenti a tali costanti, in modo da poter costruire un modello con un dominio sufficientemente popolato da soddisfare tutte le formule del sovrainsieme massimale. Nell'esempio precedente, aggiungendo una costante \(c'\) al linguaggio e l'assioma \(\exists x\st P(x) \to P(c')\) è possibile costruire un modello con un dominio con due elementi, che soddisfa \(\Gamma\).

Iniziando con la formalizzazione di questa idea, dato \(\Gamma \subseteq \FOL\) su un linguaggio \(L\) consistente, dovremo costruire un modello \(M\) di \(\Gamma\). Come da definizione di modello, \(M\) sarà una struttura \(M = \langle D, \Interp\rangle\) con \(D = \set{t \in \Terms(L')}[\Var(t) = \emptyset]\), con \(L\subseteq L'\) estensione del linguaggio \(L\) ottenuta aggiungendo nomi aggiuntivi per evitare i conflitti. Tale \(L'\) è costruito mediante un'espansione di \(L\) e di \(\Gamma\) in due sequenze \(\set{L_n}_{n\in\N}\) e \(\set{\Gamma_n}_{n\in\N}\) con \(L_0 = L\) e \(\Gamma_0 = \Gamma\), in cui a ogni passo \(n\) si aggiungono a \(L_{n-1}\) un numero infinito numerabili di nomi di costanti \(c^n_{i}\) con \(i \in \N\), e si aggiungono alla teoria i cosiddetti assiomi di Henkin, ovvero le formule \(\exists x\st \phi(x) \to \subs{\phi}{x}{c^n_{i}}\) per le formule esistenziali \(\exists x\st \phi(x)\) etichettata da un enumerazione come \(i\)-esima in \(\Gamma_{n-1}\).
...
Intuitivamente questi insiemi, detti di Henkin dal nome del logico Leon Henkin che per primo li ha introdotti, sono insiemi che contengono un testimone per ogni formula esistenziale. È importante notare che gli assiomi di Henkin, ovvero le formule della forma \(\exists x\st \phi(x) \to \subs{\phi}{x}{t}\), devono essere presenti in \(\Theta\) per ogni formula esistenziale \(\exists x\st \phi(x)\) costruibile in \(L\), anche se non presente effettivamente in \(\Theta\). Ciò sarà fondamentale nella seconda fase della costruzione poiché, quando andremo a espandere la teoria alla sua chiusura consistente, potranno essere introdotte formule esistenziali costruibili in \(L\) ma completamente \q{ortogonali} a quelle nella teoria originale che, parlando di tutt'altro, saranno chiaramente consistenti ma necessiteranno di testimoni ad hoc. Richiedendo testimoni e assiomi di Henkin solo per le formule presenti originariamente nella teoria si rischia di incorrere nuovamente nel problema citato precedentemente. Se consideriamo ad esempio il linguaggio \(L\) di prima a cui aggiungiamo un nuovo predicato unario \(Q\), ottenendo \(L' = \langle \set{c}, \emptyset, \set{P,Q}\rangle\) e la teoria \(\Gamma' = \set{\neg P(x), \exists x\st P(x), \neg Q(x) \land P(x), \neg Q(x) \land \neg P(x)}\), si ha chiaramente che la formula \(\exists x\st Q(x)\) è costruibile in \(L'\), non presente in \(\Gamma'\) e consistente con \(\Gamma'\).\footnote{Ancora una volta basta che ci siano nel dominio due elementi distinti, uno per cui vale \(Q\) e uno no.} Se andassimo ad aggiungere costanti e assiomi solo per gli esistenziali presenti in \(\Gamma'\) andremmo ad aggiungere un solo simbolo di costante, ottenendo così come dominio \(D=\set{c,c'}\) il quale è sufficiente a soddisfare \(\Gamma'\), ma non è sufficiente a soddisfare \(\Gamma' \cup \set{\exists x\st Q(x)}\), che essendo consistente con \(\Gamma'\) sarà aggiunta alla chiusura consistente di \(\Gamma'\).
...
Questa procedura garantisce anche che se \(\Gamma\) è consistente, allora anche \(\Gamma'\) è consistente. L'intuizione è che ogni formula aggiunta non interferisce ne con altre formule aggiunte ne con quelle già presenti, poiché ogni formula aggiunta aggiunge informazione solo su un termine completamente nuovo ed esclusivo, e che quindi non può introdurre conflitti. Le uniche possibili fonti di inconsistenza potrebbero dunque essere solo formule universali che in qualche modo entrano in conflitto sullo specifico nuovo testimone aggiunto ma, in tal caso, essendo che le formule aggiunte sono condizionali, l'unico modo per cui tali formule universali possano mai creare conflitto è la presenza a priori dell'antecedente esistenziale di un assioma, implicando la presenza di un conflitto già presente in \(\Gamma\), ovvero la sua inconsistenza.

Se infatti ad esempio \(\Gamma\) contiene la formula \(\forall x, \neg P(x)\) e aggiungiamo l'assioma di Henkin \(\exists x\st P(x) \to P(c_j^1)\) queste due formule non portano di per sé a nessuna inconsistenza poiché l'assioma può ancora essere soddisfatto per \(\exists x\st P(x)\) e \(P(c_j^1)\) false. L'unico modo per cui queste due formule possono creare un conflitto è se \(\Gamma\) contiene la formula \(\exists x\st P(x)\), che porta necessariamente \(P(c_j^1)\) a essere vera, ottenendo una contraddizione con \(\forall x, \neg P(x)\).
La formula \(\exists x\st P(x)\) però non rispetta la forma di un assioma di Henkin, e quindi se è presente in \(\Gamma\) è presente già prima dell'espansione, e quindi è presente già in \(\Gamma_0\). Dunque, se \(\Gamma\) è consistente, non può contenere sia \(\exists x\st P(x)\) che \(\forall x, \neg P(x)\).

Per una dimostrazione più formale di ciò si può procedere ragionando sul fatto che l'inconsistenza, essendo deduzione di \(\bot\) è una proprietà finita poiché finito è l'albero di deduzione, di conseguenza l'inconsistenza dev'essere stata introdotta per la prima volta in un certo passaggio della costruzione. A quel punto è sufficiente verificare per induzione che ogni passaggio se parte da un insieme consistente produce un insieme ancora consistente ottenendo l'assurdo che prova la consistenza del limite dell'espansione di Henkin. Non vederemo formalmente questa dimostrazione poiché è molto simile alla dimostrazione di consistenza dell'espansione di Lindenbaum che abbiamo già visto per la logica proposizionale e rivedremo ora per la costruzione dell'sovrainsieme massimalmente consistente.

Ottenuto infatti \(\Gamma'\) dell'espansione di Henkin possiamo, allo stesso modo di come visto nel \namecref{lem:lindenbaum}, possiamo andare a costruire la sua chiusura consistente \(\Gamma^*\), ovvero tramite una sequenza di insiemi basata su un enumerazione delle formule di \(L'\) della forma:
...
A questo punto ci manca solo costruire finalmente questo \(M^* = \langle D, \Interp\rangle\) la cui teoria \(\Th{M^*} = \set{\phi \in \FOL}[M^* \models \phi] = \Gamma^*\).
...
\begin{itemize}
  ...
  \item Per ogni simbolo di funzione \(f\in \FuncSyms(L') = \FuncSyms(L)\) con \(\arty(f) = n\), \(\Interp(f) = f^{M^*} \in D^{D^n}\) tale che \(f^{M^*}(t_1^{M^*}, \ldots, t_n^{M^*}) = f(t_1, \ldots, t_n)\) per ogni \(t_1^{M^*}, \ldots, t_n^{M^*} \in D\). Dunque per qualsiasi termine ground \(t\) si ha che \(\Interp(t) = t\).
  \item Per ogni simbolo di predicato \(P\in \PredSyms(L') = \PredSyms(L)\) con \(\arty(P) = n\), \(\Interp(P) = P^{M^*} =\set{(t_1, \ldots, t_n) \in D^n}[P(t_1, \ldots, t_n) \in \Gamma^*]\).
\end{itemize}
...
Ora dobbiamo mostrare solo che \(M^*\) è un modello di \(\Gamma^*\), il che implicherebbe che \(M^*\models \Gamma\) essendo sottoinsieme di \(\Gamma^*\), il che ci darebbe che \(\Gamma\) è soddisfacibile chiudendo la dimostrazione della completezza.
...
\begin{proof}
  ...
  Se \(\phi = \psi_1 \lor \psi_2\) allora, se \(\phi\) è chiusa, lo sono anche \(\psi_1\) e \(\psi_2\). Dunque per ipotesi induttiva \(M^* \models \psi_i \iff \psi_i \in \Gamma^*\) per \(i = 1,2\). Se almeno una delle due appartiene a \(\Gamma^*\) essendo chiuso per deduzione, appartiene anche \(\psi_1 \lor \psi_2\) e per semantica di \(\lor\) \(M^* \models \psi_1 \lor \psi_2\). Se invece nessuna delle due appartiene a \(\Gamma^*\) allora, per massimalità di \(\Gamma^*\), appartengono entrambe le negazioni, e quindi per chiusura per deduzione appartiene anche \(\neg \psi_1 \land \neg \psi_2\). Si ha dunque che \(\psi_1 \lor \psi_2 \notin \Gamma^*\) poiché insieme a \(\neg \psi_1 \land \neg \psi_2\) porterebbe a una contraddizione. Possiamo vederlo con la seguente dimostrazione:
  ...
  Dopo l'albero di deduzione mostrato, chiudiamo il passaggio con il collegamento semantico: per ipotesi induttiva abbiamo \(M^* \not\models \psi_1\) e \(M^* \not\models \psi_2\); dalla semantica classica della disgiunzione segue immediatamente che \(M^* \not\models \psi_1 \lor \psi_2\). Questo completa il caso della disgiunzione.
  ...
  Essendo per ipotesi \(\phi\) chiusa si ha \(FV(\psi)\subseteq \set{x}\). Non essendo \(\psi\) necessariamente chiusa non è possibile applicare immediatamente l'ipotesi induttiva. Andiamo quindi a dimostrare l'equivalenza vedendo un'implicazione per volta. Iniziamo con \(M^* \models \phi \implies \phi \in \Gamma^*\).
  ...
  Per il teorema di sostituzione, si ha inoltre
  
  \[M^* \models \forall x, \psi \iff M^* ,\sigma[x\mapsfrom d] \models \psi \iff M^*,\sigma[x \mapsfrom \den{d}[M^*][\sigma]] \models \psi  \iff M^*,\sigma \models \subs{\psi}{x}{d}\]
  essendo che gli elementi del dominio sono termini ground, e quindi \(\den{d}[M^*][\sigma] = d\). Inoltre essendo \(d\) ground è sicuramente libero per \(\psi\).

  A questo punto avendo sostituito tutte le occorrenze dell'unica potenziale variabile libera \(x\) con un termine ground \(d\), si ha che \(\subs{\psi}{x}{d}\) è una formula chiusa di \(L'\). Dunque per ipotesi induttiva \(M^* \models \subs{\psi}{x}{d} \iff \subs{\psi}{x}{d} \in \Gamma^*\) per ogni \(d\in D\).

  Ora ipotizziamo per assurdo che \(\forall x, \psi \notin \Gamma^*\). Dunque per massimalità di \(\Gamma^*\) si ha che \(\neg \forall x, \psi \in \Gamma^*\). Si ha però che \(\neg \forall x, \psi\vdash \exists x\st \neg \psi\), e dunque anche \(\exists x\st \neg \psi \in \Gamma^*\) poiché \(\Gamma^*\) è chiuso per deduzione. Essendo che \(\Gamma^*\) è un'espansione di Henkin, esiste un termine ground \(c\) di \(L'\) tale che \(\exists x\st \neg \psi \to \neg \subs{\psi}{x}{c} \in \Gamma^*\). Dunque per modus ponens si ha che \(\neg \subs{\psi}{x}{c} \in \Gamma^*\). Ma allora posso dedurre \(\bot\) da \(\Gamma^*\) tramite \(\neg E\) usando \(\subs{\psi}{x}{c}\) e \(\neg \subs{\psi}{x}{c}\), che è assurdo perché per ipotesi \(\Gamma^*\) è consistente. Dunque \(\forall x, \psi \in \Gamma^*\).
