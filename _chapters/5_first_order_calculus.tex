...
Se infatti ad esempio \(\Gamma\) contiene la formula \(\forall x, \neg P(x)\) e aggiungiamo l'assioma di Henkin \(\exists x, P(x) \to P(c_j^1)\) queste due formule non portano di per sé a nessuna inconsistenza poiché l'assioma può ancora essere soddisfatto per \(\exists x, P(x)\) e \(P(c_j^1)\) false. L'unico modo per cui queste due formule possono creare un conflitto è se \(\Gamma\) contiene la formula \(\exists x, P(x)\), che porta necessariamente \(P(c_j^1)\) a essere vera, ottenendo una contraddizione con \(\forall x, \neg P(x)\).
La formula \(\exists x, P(x)\) però non rispetta la forma di un assioma di Henkin, e quindi se è presente in \(\Gamma\) è presente già prima dell'espansione, e quindi è presente già in \(\Gamma_0\). Dunque, se \(\Gamma\) è consistente, non può contenere sia \(\exists x, P(x)\) che \(\forall x, \neg P(x)\).
...
A questo punto ci manca solo costruire finalmente questo \(M^* = \langle D, I\rangle\) la cui teoria \(\Th{M^*} = \set{\phi \in \FOL}[M^* \models \phi] = \Gamma^*\).
