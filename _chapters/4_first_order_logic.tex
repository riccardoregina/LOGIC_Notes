...
\begin{example}{}
  Sia \(\phi = \exists y \, (x < y)\) e \(t_1 = y, t_2 = z\).
  \begin{itemize}
...
Essendo la validità una proprietà universale, il modo più facile per dimostrarla è verificare che non esistano controesempi.
...
Passando al secondo, l'unica cosa che sappiamo da \(M,\sigma\models \exists x\st B(x)\) è che \(M\) dev'essere tale per cui \(B^M \neq \emptyset\). Ora, possiamo trovare una struttura con queste proprietà che non soddisfa la conclusione \(\forall x, (A(x)\to B(x))\), in particolare con \(M\) che contiene un elemento \(d\) che soddisfa \(A\) e non \(B\). A questo punto, abbiamo trovato un controesempio, da cui si ha che la formula non è valida. Poiché il primo caso ci ha detto che vi sono delle strutture che la soddisfano, \(\phi\) è soddisfacibile.
...
Consideriamo ora due formule, di cui la prima ha un albero sintattico che ha come radice un simbolo logico non proposizionale.\footnote{Si intende, un simbolo logico non appartenente alla logica proposizionale, ovvero i quantificatori.}
...
Verifichiamo la validità della prima formula, valutando se è possibile l'esistenza di un contromodello. Affinché essa sia resa falsa, si deve osservare la sua negazione, ovvero \(\forall x \st (Hat(x)\land \neg\forall y,Hat(y))\). 
Quindi, per tutti gli individui del dominio del modello, deve valere che tutti indossano il cappello e che non tutti indossano il cappello --- ovvero che esiste qualcuno che non lo indossa.
Queste due richieste, però, sono in evidente contraddizione.
Pertanto, non può esistere un contromodello e, quindi, questa formula è valida.

Adesso, verifichiamo la validità della seconda formula, cercando sempre un contromodello, che dovrà soddisfare la formula negata, ovvero \((\exists x, Hat(x)) \land (\exists y, \neg Hat(y))\).
È facile vedere che un contromodello può esistere, come, ad esempio, \(D=\set{a, b}\) e \(Hat^M=\set{a}\), e che quindi la formula non è valida.
