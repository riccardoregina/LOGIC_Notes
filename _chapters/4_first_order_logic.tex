...
Andiamo ora a caratterizzare il simbolo \(\overline{<}\) potremmo esprimere le caratteristiche di una relazione d'ordine totale. L'antiriflessività è esprimibile con l'espressione \qi{Nessun numero è minore di se stesso} con la formula \(\neg \exists x\st x \overline{<} x\), la transitività con \qi{Se \(x\) è minore di \(y\) e \(y\) è minore di \(z\), allora \(x\) è minore di \(z\)} con la formula \(\forall x, \forall y, \forall z, \left((x \overline{<} y \land y \overline{<} z)\to x \overline{<} z\right)\), e la totalità\footnote{Tecnicamente andrebbe specificata anche l'antisimmetria, ma essendo un ordine stretto è implicata} con \qi{Per ogni coppia di numeri, o il primo è minore del secondo, o sono uguali, o il secondo è minore del primo} con la formula \(\forall x, \forall y, ((x \overline{<} y)\lor(x\overline{=}y)\lor(y\overline{<} x))\).

Il potere espressivo della logica del prim'ordine è quindi molto maggiore di quello della logica proposizionale, ma questo ha anche aspetti negativi, come ad esempio l'indecidibilità dei relativi problemi di soddisfacibilità e validità, che vedremo più avanti.
...
Intuitivamente, data una struttura \(M = \langle D, \Interp \rangle\), questa si occupa di interpretare i simboli della signature di un linguaggio del prim'ordine. Essendo però che i termini possono contenere variabili, è necessario arricchire l'interpretazione mediante un assegnamento \(\sigma\) che si occupi di interpretare le variabili. 
...
Dualmente, su domini finiti la logica del prim'ordine si riduce alla logica proposizionale, in quanto ogni formula con quantificatori può essere riscritta come una formula senza quantificatori, sostituendoli con congiunzioni o disgiunzioni di formule ground.
...
Tale aggiunta è fatta poiché per quanto sia possibile assiomatizzare\footnote{Per assiomatizzazione si intende l'aggiunta di formule da soddisfare per un modello che descrivono le proprietà di un qualche simbolo non logico. Ciò permette in un certo senso di \qi{forzare} l'interpretazione di un simbolo, in quanto avendo delle formule fissate che descrivono le proprietà di tale simbolo, gli unici modelli che si potranno prendere in considerazione saranno quelli che come minimo soddisfano le proprietà assiomatizzate del simbolo in questione.
