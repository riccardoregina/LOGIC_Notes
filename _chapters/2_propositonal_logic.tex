...
\begin{proposition}{Limite superiore al numero di sottoformule}
...
\begin{definition}{Validità}
  Una formula \(\phi\) è \keyword{valida} (\(\Valid[\phi]\)) se e soltanto se per ogni modello \(m\in 2^{\AP}\) vale \(m \models \phi\).
\end{definition}
...
Ciò, oltre a evidenziare la stretta connessione tra conseguenza logica e validità (e quindi soddisfacibilità, per dualità), sottolinea come la relazione \(\models\) sia correlata alla semantica del simbolo di implicazione \(\rightarrow\).
...
Infatti, la valutazione di una formula \(\phi\) ha complessità \(\BigTheta{\abs{\AP[\phi]}}\), quindi \(\BigTheta{\abs{\phi}}\).
...
\begin{lemma}[listtitle={Estrapolato dalla dimostrazione del \Cref{thm:validity-equals-taut}}]{\infosymbol{} Estrapolato dalla dimostrazione del \Crefalt[yellow]{thm:validity-equals-taut}}
  Per ogni \(m \in 2^{AP}\), esiste un \(\alpha \in Ass(\phi)\) tale per cui \(m \models \phi \iff \Val{\alpha}[\phi]=1\).
\end{lemma}

\begin{proof}
  Per ogni modello \(m\), possiamo definire un assegnamento \(\alpha_{m}\) t.c.
  \[\alpha_{m}(P) = \begin{cases}
    1, & P \in m\\
    0, & \text{ altrimenti}
  \end{cases}\]
  Facciamo vedere, per induzione strutturale, che \(m\models \phi \iff \Val{\alpha_m}[\phi] = 1\).
  \begin{description}
...
\begin{lemma}[listtitle={Estrapolato dalla dimostrazione del \Cref{thm:validity-equals-taut}}]{\infosymbol{} Estrapolato dalla dimostrazione del \Crefalt[yellow]{thm:validity-equals-taut}}
  Per ogni \(\alpha \in Ass(\phi)\), esiste un \(m \in 2^{AP}\) tale per cui \(m \models \phi \iff \Val{\alpha}[\phi]=1\).
\end{lemma}

\begin{proof}
  Per ogni assegnamento \(\alpha\), è possibile definire un modello \(m_{\alpha}\) t.c.
...
Procediamo adesso, per contrapposizione, alla dimostrazione del teorema. Quindi, vogliamo dimostrare \(\not\models \phi \iff \neg \Taut(\phi)\).
\begin{proof}[Dimostrazione (\ref{thm:validity-equals-taut})]
  \begin{description}
    ...
    \item[\qi{\(\impliedby\)}]
      Sia \(\alpha\) un assegnamento per \(\phi\) t.c. \(\Val{\alpha}[\phi]=0\).
...
\begin{proof}
  ...
  Applicando la sostituzione \(\sigma: A \to m\models \Gamma, B \to m\models \phi, C \to m\models \psi\), si ottiene 
...
\begin{definition}{Conjunctive Normal Form}
  ...
  \[\phi = \bigwedge_{i=1}^k\left(\bigvee_{j=1}^{r_i} l_i^j\right)\]
  dove \(l_{ij} \in \mathcal{L}\) per ogni \(i\) e \(j\).
\end{definition}
...
\begin{note}{}
  ...
  Ciò è possibile dal momento in cui tutte le formula insoddisfacibili sono equivalenti a una contraddizione.
\end{note}
