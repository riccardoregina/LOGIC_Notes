...
Tra le varie applicazioni, quella su cui ci concentreremo è la specifica di problemi decisionali, ovvero problemi che possono essere risolti con una risposta binaria: sì o no.

In particolare, andremo a vedere come le sintassi e le semantiche dei linguaggi logici possano essere utilizzate per rappresentare gli input, gli output e una relazione tra di essi, in modo tale che la congiunzione delle formule risultanti sia soddisfacibile se e solo se il problema da risolvere ha risposta affermativa.
...
Una volta trovato un algoritmo corretto, completo e terminante per risolvere la soddisfacibilità di formule proposizionali, se troviamo una \keyword{riduzione}\footnote{Per riduzione si intende una codifica di un'istanza qualsiasi del problema in un'istanza di \(\Sat\) --- un'istanza di \(\Sat\) è una formula proposizionale --- tale per cui la soluzione di quest'ultima ci fornisca la soluzione per l'istanza del problema originale.} di un qualsiasi problema a \(\Sat\), allora troviamo un algoritmo per risolvere quel problema.
...
Per quanto riguarda l'output, un modo alternativo alla sequenza di vertici per descrivere un ciclo è quello di rappresentarlo come sequenza di archi. Vorremo dunque mostrare che esista un \(S\subseteq E\) tale che \(S\) contiene un ciclo. Partiamo dal definire le variabili per l'output come \(Y_{u,v}\) per ogni \(u,v \in V\). Dunque possiamo definire 
\[\phi_S \eqdef \bigwedge_{u,v \in V} Y_{u,v}\to X_{u,v}\]
Tale formula ci dice che ogni elemento di \(S\) deve stare in \(E\), in quanto sarà falsa solo se l'output conterrà qualche arco \(Y_{u,v}\in S\) che non corrisponde a nessun arco \(X_{u,v}\in E\). Inoltre, per far sì che \(S\) contenga un ciclo, \(S\) non può essere vuoto, e questo è esprimibile con
\[\phi_{\neq \emptyset} \eqdef \bigvee_{u,v\in V} Y_{u,v} \]

Ci manca solo da definire i vincoli che legano \(S\) al fatto che contenga un ciclo. La formula che ci permette di esprimere questo vincolo è la seguente:
\[\phi_{cyc} \eqdef \bigwedge_{u,v} (Y_{u,v}\to \bigvee_{z \in V} Y_{v,z})\]

che ci dice che, affinchè vi sia un ciclo nell'output \(S\), ogni arco di output \(Y_{u,v}\) deve avere almeno un arco \(Y_{v,z}\) che parte dal vertice \(v\) e arriva in un altro vertice \(z\), che potrebbe essere anche lo stesso \(u\) o lo stesso \(v\).
Se questa formula è vera, allora, se \(V\) è finito, nell'output \(S\) vi sarà necessariamente una catena chiusa di archi. Infatti, se \(V\) fosse infinito, allora \(S\) potrebbe anche non contenere un ciclo.
...
Per questo problema sono noti anche algoritmi polinomiali, dunque questa riduzione non è particolarmente utile, ma è un esempio semplice di come si possa rappresentare un problema con una formula proposizionale.
...
\[\phi_{f} \eqdef \bigwedge_{v\in V} \bigvee_{c\in C} \left( Y_v^c \land \bigwedge_{c'\in C\setminus\set{c}} \neg Y_v^{c'} \right)\]
...
\begin{definition}{Proprietà}
  Sia \(L\) un linguaggio di \(\FOL\) e sia \(\mathcal{M}\) la classe delle strutture con \(m \in \mathcal{M}\) se \(m = (D,I)\) per \(L\). Una \keyword{proprietà} \(P\) è un qualsiasi un sott'insieme di \(\mathcal{M}\).
\end{definition}

Una proprietà è quindi un insieme di strutture accomunate da un qualcosa.
...
\begin{definition}{Espressibilità}
  Sia \(L\) un linguaggio di \(\FOL\) e sia \(\mathcal{M}\) la classe delle strutture con \(m \in \mathcal{M}\) se \(m = (D,I)\) per \(L\). Una proprietà \(P\) è \keyword{esprimibile} in \(L\) se esiste una formula \(\phi \in \FOL\) tale che \(\set{m\in\mathcal{M}}[ m \models \phi] = P\)
\end{definition}

Una proprietà è quindi esprimibile se esiste una formula che è soddisfatta da tutte le strutture della proprietà.
Questo problema è particolarmente interessante per l'informatica poiché ci permette di capire quanto potere espressivo sia necessario per esprimere determinate proprietà nella modellazione di basi di conoscenza. Intuitivamente, l'espressività di un linguaggio è inversamente proporzionale alla complessità computazionale dei problemi di decisione di tale linguaggio, quindi è importante capire quali siano le proprietà che possiamo esprimere con un linguaggio per trovare un giusto equilibrio tra potere espressivo e complessità computazionale.
...
Chiaramente quindi \(\den{\phi_{lin}}\), ovvero la denotazione di \(\phi_{lin}\) è proprio la proprietà di essere linearmente ordinato.
...
\[\phi' \eqdef \forall x, (\exists y x< y \to \exists z (x<z \land \forall w, (x<w \to \neg(w<z))))\]
...
Passando a qualche risultato negativo, abbiamo che \((\Q,<)\) è indistinguibile da \((\R,<)\), ovvero non esiste una formula \(\phi\) tale che \((\Q,<)\models \phi\) e \((\R,<)\not\models \phi\). Questa è conseguenza diretta del teorema di Löwenhaim-Skolem che recita quanto segue.
\begin{theorem}{Löwenhaim-Skolem}
  Se un insieme di formule chiuse \(\Gamma\) è al più numerabile e soddisfacibile, allora \(\Gamma\) ha un modello al più numerabile.
\end{theorem}

La proprietà \(\den{\phi_{DNS}}\) contiene sicuramente il modello \((\R,<)\), in quanto ordine lineare denso. Supponiamo che \((\R,<)\) soddisfi anche la formula \(\phi\). Pertanto, \(\Gamma = \set{\phi} \cup \phi_{DNS}\) è un insieme soddisfacibile ed al più numerabile di formule chiuse. Quindi, per il teorema di Löwenhaim-Skolem, esiste un modello \(M\) al più numerabile che soddisfa \(\Gamma\). Poichè \(M\models \phi_{DNS}\), \(M\) è un ordine lineare denso. Inoltre, \(M\) è al più numerabile. Tale modello non può essere \((\R,<)\) e si dimostra essere \((\Q, <)\).
Pertanto, \((\R, <)\models \phi \implies (\Q, <)\models \phi\) e si dimostra anche il verso contrario.

Intuitivamente, il teorema di Löwenhaim-Skolem vale per la costruzione vista per il teorema di completezza, poiché la costruzione del modello dell'insieme soddisfacibile \(\Gamma^*\) portava ad avere un dominio al più numerabile, poiché abbiamo aggiunto un numero numerabile di insiemi numerabile di costanti, ottenendo ancora un insieme enumerabile, se lo era quello di partenza.

Questo ci dice che ci sono limiti in ciò che il prim'ordine può esprimere.

\begin{theorem}{Compattezza}
  Per ogni \(\Gamma\subseteq\FOL\) (formule chiuse) e per ogni formula \(\phi\) (chiusa), vale che
  
  \(\Gamma \models \phi \iff \) esiste \(\Gamma_0 \subseteq \Gamma\) finito tale che \(\Gamma_0 \models \phi\)
\end{theorem}
...
\begin{theorem}{Compattezza Alternativa}
  ...
  \(\Gamma \in\Unsat \iff\) esiste \(\Gamma_0 \subseteq \Gamma\) finito tale che \(\Gamma_0 \in\Unsat\)
  o, equivalentemente, 
  \(\Gamma \in\Sat \iff\) per ogni \(\Gamma_0 \subseteq \Gamma\) finito, \(\Gamma_0 \in\Sat\)
\end{theorem}
...
\[q_k^I = \set{(x,y)\in\N^2}[{(\pi(x))}_1 = k \land {(\pi(x))}_2 = y ]\]
ovvero \(q_k^I\) è l'insieme di tutte le coppie \((x,y)\) tali che alla \(i\)-esima configurazione dell'esecuzione, la macchina si trovi nello stato \(q_k\) e la testina sia posizionata sulla cella \(j\).
...
\[\sigma_l^I = \set{(x,y)\in\N^2}[{(\pi(x))}_3(y) = \sigma_l] \]
ovvero \(\sigma_l^I\) è l'insieme di tutte le coppie \((x,y)\) tali che alla \(i\)-esima configurazione dell'esecuzione, il nastro contenga in posizione \(y\) il simbolo \(\sigma_l\).
...
\begin{equation}
  \phi_1 \eqdef \forall x, (s(p(x)) = x \land (\neg(x = 0) \to p(s(x)) = x))
\end{equation}
...
Per completare la dimostrazione, è necessario mostrare che \(\models \phi_M\) se e solo se \(M\) termina, ovvero se esiste un \(i \in \N\) tale che \({(\pi_M(i))}_1 = n\).
