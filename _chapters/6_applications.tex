...
\todo{Questo capitolo è da intendere dopo la logica del primo ordine, quindi si faranno riferimenti anche a essa.}
...
\begin{definition}{Proprietà}
  Sia \(L\) un linguaggio di \(\FOL\) e sia \(\mathcal{M}\) la classe delle strutture con \(m \in \mathcal{M}\) se \(m = (D,I)\) per \(L\). Una \textbf{proprietà} \(P\) è un qualsiasi un sott'insieme di \(\mathcal{M}\).
\end{definition}
...
\begin{theorem}{Löwenhaim-Skolem}
  Se \(\Gamma\) è un insieme di formule chiuse al più numerabile e soddisfacibile, allora \(\phi\) ha un modello al più numerabile.
\end{theorem}

Se una formula \(\phi\) è una formula che soddisfa \((\R,<)\) chiaramente sarà soddisfatto anche \(\set{\phi, \phi_{DNS}}\) che è un insieme di formule chiuse al più numerabile e soddisfacibile, quindi per il teorema di Löwenhaim-Skolem esiste un modello al più numerabile che soddisfa \(\set{\phi}\). Tale modello non può essere \((\R,<)\) poiché non è al più numerabile, quindi deve essere \((\Q,<)\).
...
\[\phi_n \eqdef \neq\exists x_1, \ldots, x_n ( x_1 = s \land x_n = t \land \bigwedge_{1 \leq i < n} E(x_i, x_{i+1}))\]
