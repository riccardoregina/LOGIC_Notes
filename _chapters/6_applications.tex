...
\begin{definition}{Espressibilità}
  Sia \(L\) un linguaggio di \(\FOL\) e sia \(\mathcal{M}\) la classe delle strutture con \(m \in \mathcal{M}\) se \(m = (D,I)\) per \(L\). Una proprietà \(P\) è \textbf{esprimibile} in \(L\) se esiste una formula \(\phi \in \FOL\) tale che \(\set{m\in\mathcal{M}}[ m \models \phi] = P\)
\end{definition}
